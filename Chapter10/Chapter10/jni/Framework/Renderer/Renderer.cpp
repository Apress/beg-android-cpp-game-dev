...
namespace Framework
{
	...
	void Renderer::Draw(Renderable* pRenderable)
	{
		...
		if (pRenderable)
		{
			...
			Shader* pShader = pRenderable->GetMaterial()->GetShader();
	...
	void Renderer::BuildFrustumPlanes(Plane frustumPlanes[])
	{
		...
		const Matrix4& cameraMatrix = m_cameraTransform.GetMatrix();
		Vector3 cameraRight(cameraMatrix.m_m[0], cameraMatrix.m_m[1], cameraMatrix.m_m[2]);
		Vector3 cameraUp(cameraMatrix.m_m[4], cameraMatrix.m_m[5], cameraMatrix.m_m[6]);
		Vector3 cameraForward(cameraMatrix.m_m[8], cameraMatrix.m_m[9], cameraMatrix.m_m[10]);
		Vector3 cameraPosition(cameraMatrix.m_m[12], cameraMatrix.m_m[13], cameraMatrix.m_m[14]);
	...
	void Renderer::Update()
	{
		...
		{
			m_cameraTransform.GetInverseMatrix(m_viewMatrix);
